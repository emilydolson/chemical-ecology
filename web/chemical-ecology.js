...
var STATIC_BASE = 1024,
    ...
    STACKTOP = STACK_BASE,
    STACK_MAX = 5309744,
    DYNAMIC_BASE = 5309744,
    DYNAMICTOP_PTR = 66656;
...
var ASM_CONSTS = [function($0) { msg = UTF8ToString($0); if (typeof alert == "undefined") { globalThis.alert = console.log; } alert(msg); },
 ...
 function($0) { emp_d3.objects[$0] = d3.select(); },
 ...
 function($0, $1, $2) { const id = $0; const selector = UTF8ToString($1); const new_id = $2; var new_selection = emp_d3.objects[id].selectAll(selector); emp_d3.objects[new_id] = new_selection; },
 function($0, $1, $2) { const selection_id = $0; const in_string = UTF8ToString($1); const new_id = $2; var fn = emp_d3.find_function(in_string); if (typeof fn === "function"){ var update_sel = emp_d3.objects[selection_id].data(emp_i.__incoming_array, fn); } else { var update_sel = emp_d3.objects[selection_id].data(emp_i.__incoming_array); } emp_i.__incoming_array = []; emp_d3.objects[new_id] = update_sel; },
 function($0, $1, $2) { const selection_id = $0; const type_str = UTF8ToString($1); const new_id = $2; var append_selection = emp_d3.objects[selection_id].enter() .append(type_str); emp_d3.objects[new_id] = append_selection; },
 function($0, $1, $2) { const id = $0; const other_id = $1; const new_id = $2; emp_d3.objects[new_id] = emp_d3.objects[id].merge(emp_d3.objects[other_id]); },
 function($0, $1, $2) { emp_d3.objects[$0].attr(UTF8ToString($1), function(d, i, k) { return emp.Callback($2, d, i, k); }); },
 function($0, $1, $2, $3) { const id = $0; const name = UTF8ToString($1); const fun_id = $2; const priority = $3; if (priority) { emp_d3.objects[id].style( name, function(d, i) { return emp.Callback(fun_id, d, i); }, "important" ); } else { emp_d3.objects[id].style( name, function(d, i) { return emp.Callback(fun_id, d, i); } ); } },
 function($0, $1, $2, $3) { const id = $0; const type = UTF8ToString($1); const func_id = $2; const capture = $3; emp_d3.objects[id].on( type, function(d, i) { emp.Callback(func_id, d, i); }, capture ); },
 ...
 function($0, $1) { var curr_array = emp_i.__incoming_array; var depth = 0; while (curr_array.length > 0) { var next_index = getValue($1+(depth*4), "i32"); depth += 1; curr_array = curr_array[next_index]; } for (i=0; i<$0; i++) { var new_obj = {}; curr_array.push(new_obj); } },
 function($0, $1, $2, $3, $4) { var curr_array = emp_i.__incoming_array; var depth = 0; while (curr_array[0].length > 0) { var next_index = getValue($4+(depth*4), "i32"); depth += 1; curr_array = curr_array[next_index]; } if (UTF8ToString($1) == "string") { curr_array[$3][UTF8ToString($2)] = UTF8ToString($0); } else { curr_array[$3][UTF8ToString($2)] = getValue($0, UTF8ToString($1)); } },
 function($0) { emp_i.object_queue = []; emp_i.curr_obj = emp_i.cb_args[$0]; },
 function($0) { return emp_i.curr_obj[UTF8ToString($0)]; },
 function() { emp_i.curr_obj = emp_i.object_queue.pop(); },
 function($0) { emp_i.cb_return = UTF8ToString($0); },
 function($0) { return emp_i.cb_args[$0]; }];
...
function _emscripten_asm_const_sync_on_main_thread_di(code, a0) {
  return ASM_CONSTS[code](a0);
}
...
function _GetNewWeight(old){ var new_weight = prompt("New weight:", old); if (new_weight == null || new_weight == "") { return old; } return +new_weight; }
...
var ___em_js__GetNewWeight = Module["___em_js__GetNewWeight"] = function() {
  assert(runtimeInitialized, 'you need to wait for the runtime to be ready (e.g. wait for main() to be called)');
  assert(!runtimeExited, 'the runtime was exited (use NO_EXIT_RUNTIME to keep it alive after main() exits)');
  return Module["asm"]["___em_js__GetNewWeight"].apply(null, arguments)
};
