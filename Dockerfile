...
COPY . /opt/chemical-ecology

RUN \
  pip3 install -r /opt/chemical-ecology/docs/requirements.txt \
    && \
...
RUN \
  chgrp --recursive user /opt/chemical-ecology \
    && \
  chmod --recursive g+rwx /opt/chemical-ecology \
    && \
  echo "user granted permissions to /opt/chemical-ecology"

USER user

RUN \
  cd /opt/chemical-ecology \
    && \
