...
#diet, Diet, Pollination, pollinator
networks <- search_networks(query = 'diet') %>% get_collection
for (i in 1:length(networks)){
    net_tbl <- networks[[i]]$interactions

	if ((net_tbl$attribute.name[1] == "presence/absence") | (net_tbl$attribute.name[1] == "pollinator recorded on a flower") | (net_tbl$attribute.name[1] == "mean interaction frequency") | (net_tbl$network_id[1] > 5000)){
		next
	}
