...
def plot(dir, class_num):
    ...
    for measure in measures:
        ...
        min_Variance = data['Variance'].min() 
        if max_Variance > best_Variance:
            best_Variance = max_Variance
        if min_Variance < worst_Variance:
            worst_Variance = min_Variance
    ...
    for measure in measures:
        ...
        for i, clear in enumerate(clear_values):
            ...
            heatmap_data = subset.pivot(index="Seeding", columns="Diffusion", values="Variance")
        ...
        fig.suptitle(f"Adaptive Variances for a class {class_num} matrix", fontsize=20)
        ...
        # Create a colorbar
        norm = plt.Normalize(worst_Variance, best_Variance)
